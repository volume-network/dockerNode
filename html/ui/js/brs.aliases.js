...
BRS = (function (BRS, $, undefined) {
        ...
        if (alias) {
            ...
            BRS.sendRequest("getAlias", {
                ...
                if (response.errorCode) {
                    ...
                    else if ((aliasURI = /acct:(.*)@volume/.exec(response.aliasURI)) || (aliasURI = /nacc:(.*)/.exec(response.aliasURI))) {
    ...
    BRS.forms.setAlias = function ($modal) {
        ...
        if (data.type === "account") {
            if (!(/acct:(.*)@volume/.test(data.aliasURI)) && !(/nacc:(.*)/.test(data.aliasURI))) {
                if (/^(VOL\-)/i.test(data.aliasURI)) {
                    ...
                    if (!address.set(data.aliasURI)) {
                        ...
                        data.aliasURI = "acct:" + data.aliasURI + "@volume";
                    }
                }
    ...
    function setAliasType(type, uri) {
        ...
        if (type == "uri") {
            ...
            $("#register_alias_uri").val("").mask("VOL-****-****-****-*****");

            if (uri) {
                var match = uri.match(/acct:(.*)@volume/i);
                if (!match) {
                ...
                else if (!/^VOL\-[A-Z0-9]{4}\-[A-Z0-9]{4}\-[A-Z0-9]{4}\-[A-Z0-9]{5}/i.test(uri)) {
                    uri = BRS.accountRS;
                }
                    ...
                    if ("priceNQT" in response) {
                        if (response.buyer == BRS.account) {
                            message = $.t("alias_sale_direct_offer", {
                                "volume": BRS.formatAmount(response.priceNQT)
