       ...
       $('#request_burst_immutable').change(function() {
        ...
        if($(this).is(":checked")) {
         ...
         $("#generate_qr_button").on("click", function(e) {
            ...
            if(suggested_fee){
                $("#request_burst_qrcode_response").html('<img src="/vol?requestType=generateSendTransactionQRCode&receiverId='+receiverId+'&amountNQT='+amountNQT+'&feeSuggestionType='+suggested_fee+immutable+'"/>');
                $("#request_burst_fee_response").html(suggested_fee.charAt(0).toUpperCase() + suggested_fee.slice(1));
            }
            else{
                $("#request_burst_qrcode_response").html('<img src="/vol?requestType=generateSendTransactionQRCode&receiverId='+receiverId+'&amountNQT='+amountNQT+'&feeNQT='+feeNQT+immutable+'"/>');
            ...
            $("#request_burst_amount_response").html($("#request_burst_amount").val() + " VOL");
