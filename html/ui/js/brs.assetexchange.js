...
var BRS = (function(BRS, $, undefined) {
    ...
    BRS.forms.addAssetBookmark = function($modal) {
        ...
        if (!/^\d+$/.test(data.id) && !/^VOL\-/i.test(data.id)) {
            return {
                "error": $.t("error_asset_or_account_id_invalid")
            };
        }

        if (/^VOL\-/i.test(data.id)) {
    ...
    $("#asset_order_modal").on("show.bs.modal", function(e) {
        ...
        if (orderType == "buy") {
            description = $.t("buy_order_description", {
                ...
                "volume": BRS.formatAmount(priceNQTPerWholeQNT)
            });
            tooltipTitle = $.t("buy_order_description_help", {
                "volume": BRS.formatAmount(priceNQTPerWholeQNT, false, true),
        ...
        $("#asset_order_total").html(totalNXT + " VOL");
        $("#asset_order_fee_paid").html(BRS.formatAmount(feeNQT) + " VOL");
