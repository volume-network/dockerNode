...
var BRS = (function(BRS, $, undefined) {
    ...
    BRS.login = function(password, callback) {
        if (!password.length) {
            ...
            if ( BRS.hasLocalStorage ) {
                if ( BRS.settings.remember_passphrase ) {
                    localStorage.setItem("volume.passphrase", $("#login_password").val());
                }
                else {
                    localStorage.removeItem("volume.passphrase");
                }
            }
        }
        ...
        BRS.sendRequest("getBlockchainStatus", function(response) {
            ...
            if (password.trim().toUpperCase().substring(0, 4) === "VOL-" && password.length === 24) {
